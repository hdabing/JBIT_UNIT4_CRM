<%@ page import="java.util.List,entity.Roleauthor" %>
...
<!--
//获取服务器session的json字符串
var str_json="<%=session.getAttribute("str_json") %>";
//将接收到的字符转换为json格式
...
function initialise() {
	...
	//利用session里面存储的json字符串转换为json类型的数据，然后做对比
	if(json.L01==0){
		L01 = theMenu.addChild(l2ID, "Folder", "营销管理", "", "营销管理...");
		if(json.L0101==0){
			L0101 = theMenu.addChild(L01, "Document", "销售机会管理", "queryListchances", "销售机会管理...");
		}
		if(json.L0102==0){
			L0102 = theMenu.addChild(L01, "Document", "客户开发计划", "queryListchancesofplan", "客户开发计划...");
		}
	}
	...
	if(json.L02==0){
		...
		if(json.L0201==0){
			L0201 = theMenu.addChild(L02, "Document", "客户信息管理", "queryListcustomer", "客户信息管理...");
		}
		if(json.L0202==0){
			L0202 = theMenu.addChild(L02, "Document", "客户流失管理", "queryListcustomerlost", "客户流失管理...");
		}
	}
	...
	if(json.L03==0){
		...
		if(json.L03==0){
			L0301 = theMenu.addChild(L03, "Document", "服务创建", "addServicelist", "服务创建...");
		}
		if(json.L03==0){
			L0302 = theMenu.addChild(L03, "Document", "服务分配", "queryListgrantservicelist", "服务分配...");
		}
		if(json.L03==0){
			L0303 = theMenu.addChild(L03, "Document", "服务处理", "queryListoperatorservicelist", "服务处理...");
		}
		if(json.L03==0){
			L0304 = theMenu.addChild(L03, "Document", "服务反馈", "queryListbackservicelist", "服务反馈...");
		}
		if(json.L03==0){
			L0305 = theMenu.addChild(L03, "Document", "服务归档", "queryListarchservicelist", "服务归档...");
		}
	}
	...
	if(json.L04==0){
		...
		if(json.L04==0){
			L0401 = theMenu.addChild(L04, "Document", "客户贡献分析", "queryListordersamount", "客户贡献分析...");
		}
		if(json.L04==0){
			L0402 = theMenu.addChild(L04, "Document", "客户构成分析", "queryListcustomercount", "客户构成分析...");
		}
		if(json.L04==0){
			L0403 = theMenu.addChild(L04, "Document", "客户服务分析", "queryListservicelistCount", "客户服务分析...");
		}
		if(json.L04==0){
			L0404 = theMenu.addChild(L04, "Document", "客户流失分析", "queryListcustomeragreelost", "客户流失分析...");
		}
	}
	...
	if(json.L05==0){
		...
		if(json.L05==0){
			L0501 = theMenu.addChild(L05, "Document", "客户等级管理", "queryListcustomerlevel", "客户等级管理...");
		}
		if(json.L05==0){
			L0502 = theMenu.addChild(L05, "Document", "服务类型管理", "queryListservice", "服务类型管理...");
		}
		if(json.L05==0){
			L0503 = theMenu.addChild(L05, "Document", "客户地区管理", "queryListarea", "客户地区管理...");
		}
		if(json.L05==0){
			L0504 = theMenu.addChild(L05, "Document", "查询产品信息", "queryListproduct", "查询产品信息...");
		}
		if(json.L05==0){
			L0505 = theMenu.addChild(L05, "Document", "查询库存", "queryListstore", "查询库存...");
		}
	}
	...
	if(json.L06==0){
		...
		if(json.L06==0){
			L0601 = theMenu.addChild(L06, "Document", "用户管理", "queryListusers", "用户管理...");
		}
		if(json.L06==0){
			L0602 = theMenu.addChild(L06, "Document", "角色管理", "queryListrole", "角色管理...");
		}
	}
